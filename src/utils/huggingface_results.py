...
def upload_artifact_if_enabled(upload_config: ArtifactUploadConfig, output_dir: str | Path) -> None:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    zip_name = f"STFT256k_Raw2049_FreqAttn_MobileNetV2_Artifacts_{timestamp}.zip"
    ...
    path_in_repo = f"STFT256k_Raw2049_FreqAttn_MobileNetV2_Artifacts_{timestamp}.zip"
...
def upload_results_artifact(config_path: str | Path = "config/artifact_upload_config.json") -> None:
    upload_config = ArtifactUploadConfig.from_json(config_path)
    upload_artifact_if_enabled(upload_config, upload_config.source_dir)
