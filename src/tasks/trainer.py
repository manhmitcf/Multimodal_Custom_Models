...
class MultimodalTrainer:
    ...
    def fit_then_test(self) -> dict[str, Any]:
        ...
        with summary_path.open("w", newline="", encoding="utf-8") as handle:
            ...
            val_acc_val = best_val_result.accuracy if best_val_result else validation.accuracy
            val_map_val = best_val_result.mAP if best_val_result else validation.mAP
            writer.writerow([
                round(best_score, 4),
                round(val_acc_val, 4),
                round(val_map_val, 4),
