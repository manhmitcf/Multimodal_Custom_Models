...
*(Nhánh: `exp/stft256k-raw2049-freqattn-mobilenetv2-se-fbgf` - SE 1D Channel Recalibration)*

Tài liệu này giải thích chi tiết ý nghĩa từng thông số trong file cấu hình JSON [`config/train_config.json`](file:///C:/Users/manhm/Desktop/Multimodal_Custom_Models/src/config/train_config.json) và [`config/artifact_upload_config.json`](file:///C:/Users/manhm/Desktop/Multimodal_Custom_Models/src/config/artifact_upload_config.json) của nhánh **`exp/stft256k-raw2049-freqattn-mobilenetv2-se-fbgf`**, giúp bạn dễ dàng chỉnh sửa và tùy biến cho các kịch bản thử nghiệm khác nhau.
...
* **`video_by_backbone`**: Từ điển ánh xạ từng loại visual backbone tới file trọng số `video_best.pt` tương ứng:
  * `"mobilenet_v2"` ⭐ *(Mặc định)*: `"../checkpoints/MobileNetV2_holdout_random_sample_20260729_153012/DL_video/checkpoint/mobilenet_v2/video_best.pt"`
...
### 🔹 Khối `model` (Kiến trúc Mô hình & SE 1D Channel Recalibration)
* **`video_backbone`**: `"mobilenet_v2"` (Sử dụng backbone **MobileNetV2** pre-trained trích vector 1280-dim $[B, 1280]$).
* **`video_se_recalibration`**: `true` ⭐ *(Cơ chế 1 Mới)*: Bật module **Squeeze-and-Excitation 1D Channel Recalibration (`SERecalibration1D`)** tự động nhân trọng số chú ý $[0, 1]^{1280}$ cho vector MobileNetV2 trước khi đưa vào dung hợp.
* **`encoder_mode`**:
...
* **`fusion_learning_rate`**: Tốc độ học tối ưu `0.0001` ($10^{-4}$) dành riêng cho Audio CNN và Fusion Head.
...
* **`path_prefix`**: Tên tiền tố thư mục trên Hugging Face (`"stft256k_raw2049_freqattn_mobilenetv2_se_fbgf"`).
...
```json
{
  ...
  "zip_path": "checkpoint/STFT256k_Raw2049_FreqAttn_MobileNetV2_SE_Artifacts.zip",
  "repo_id": "manhmitcf/fish_result",
  "repo_type": "dataset",
  "path_in_repo": "STFT256k_Raw2049_FreqAttn_MobileNetV2_SE_Artifacts.zip",
  "create_repo": true
}
```
...
### 3. Tắt/Bật SE 1D Channel Recalibration
Chỉnh `"video_se_recalibration": false` nếu muốn chạy ablation study bỏ qua lớp SE:
```json
"model": {
  "video_se_recalibration": false
}
```

### 4. Khởi chạy Pipeline trên Server Marimo
```bash
